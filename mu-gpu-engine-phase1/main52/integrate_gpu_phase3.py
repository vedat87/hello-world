...
# BMD::Transform: preserve editor/debug path, skip eager CPU vertex transforms in release gameplay.
...
# RenderMesh only: lazily materialize normals only when legacy lighting/chrome needs them,
# then use GPU draw after Main has selected texture/blend/depth state.
def patch_render_mesh(fn):
    # Legacy lighting loop reads IntensityTransform/NormalTransform.
    ...
    # Chrome/Oil texture-coordinate generation also reads transformed normals.
    ...
    if chrome_anchor in fn and 'GPU_NORMALS_FOR_CHROME' not in fn:
        repl = chrome_anchor + '\r\n\t\t// GPU_NORMALS_FOR_CHROME: legacy chrome UV code still consumes transformed normals.\r\n\t\tif(BMDGpuBridge::IsActive(this))\r\n\t\t\tBMDGpuBridge::EnsureCpuNormals(this, i);'
        fn = fn.replace(chrome_anchor, repl, 1)

    draw_anchor = '    // ver 1.0 (triangle)'
    assert draw_anchor in fn
    if 'GPU_PHASE3_DRAW' not in fn:
        block = '''    // GPU_PHASE3_DRAW: texture/blend/depth state above remains owned by the original Main.
    ...
    // Unsupported/special pass: reconstruct only the CPU scratch data that the old path needs.
    if(BMDGpuBridge::IsActive(this))
    {
...
# Alternative renderer stays fully legacy in Phase3.
def patch_alt(fn):
...
# Effect renderer consumes transformed positions.
def patch_effect(fn):
    anchor = '\tif(m->NumTriangles <= 0) return;'
    if anchor not in fn:
        anchor = '\tif(m->NumTriangles == 0) return;'
...
'''.replace('\n','\r\n')
        fn = fn.replace(anchor, anchor + block, 1)
    return fn
# RenderMeshEffect is followed later by RenderBody in this source.
try:
    c = patch_function(c, 'void BMD::RenderMeshEffect', 'void BMD::RenderBody(', patch_effect)
except ValueError:
    pass

# Collision and light-map helpers are CPU consumers.
...
# Translated renderer is kept on legacy scratch arrays for visual parity.
...
# Release GPU buffers before Mesh_t storage is destroyed.
...
# Physics cloth explicitly needs CPU positions.
...
# Create(): before it starts copying VertexTransform into cloth vertices.
...
# SetFixedVertices(): ensure the current BMD owns valid scratch positions.
...
# Add the four GPU files directly to the user's Main project. OpenGL is already linked by the project.
...
if 'source\\GpuSkinningLegacy.cpp' not in proj:
    proj = proj.replace('    <ClCompile Include="source\\ZzzBMD.cpp" />',
                        '    <ClCompile Include="source\\ZzzBMD.cpp" />\n    <ClCompile Include="source\\GpuSkinningLegacy.cpp" />\n    <ClCompile Include="source\\BMDGpuBridge.cpp" />', 1)
