...
void ReceiveChangePlayer(BYTE* ReceiveBuffer)
{
	...
	case 8:
		...
		else
		{
			...
			//==CustomPet
			if (gCustomPet2.CheckCustomPetByItem(c->Helper.Type - MODEL_ITEM))
			{
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		if (FindText(Temp, "webzen") == false)
		{
			...
			for (int j = 0; j < Data2->s_BuffCount; ++j)
			{
				...
				g_ConsoleDebug->Write(MCD_RECEIVE, "1 Check ID : %s, Buff : %d", c->ID, static_cast<int>(Data2->s_BuffEffectState[j]));
			}
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		if (FindText(Temp, "webzen") == false)
		{
			...
			for (int j = 0; j < Data2->s_BuffCount; ++j)
			{
				...
				g_ConsoleDebug->Write(MCD_RECEIVE, "3 Check ID : %s, Buff : %d", c->ID, static_cast<int>(Data2->s_BuffEffectState[j]));
			}
...
void ReceiveCreateMonsterViewport(BYTE* ReceiveBuffer)
{
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		for (int j = 0; j < Data2->s_BuffCount; ++j)
		{
			...
			g_ConsoleDebug->Write(MCD_RECEIVE, "2 Check ID : %s, Buff : %d", c->ID, static_cast<int>(Data2->s_BuffEffectState[j]));
		}
...
void ReceiveCreateSummonViewport(BYTE* ReceiveBuffer)
{
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		for (int j = 0; j < Data2->s_BuffCount; ++j)
		{
			...
			g_ConsoleDebug->Write(MCD_RECEIVE, "4 Check ID : %s, Buff : %d", c->ID, static_cast<int>(Data2->s_BuffEffectState[j]));
		}
...
void SetPlayerBow(CHARACTER* c)
{
	...
	case BOWTYPE_BOW:
	{
		if (c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(Hero->Helper.Type - 1171) == 5)
		{
	...
	{
		if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(Hero->Helper.Type - 1171) == 5) && !c->SafeZone)
		{
...
BOOL ReceiveMagic(const BYTE* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	case MASTER_SKILL_ADD_ICE_IMPROVED1:
	case MASTER_SKILL_ADD_ICE_IMPROVED2:
		CreateEffect(MODEL_ICE, to->Position, so->Angle, so->Light);

		for (int i = 0; i < 5; i++)
			CreateEffect(MODEL_ICE_SMALL, to->Position, so->Angle, so->Light);
	...
	case AT_SKILL_ENERGYBALL:
		if (so->Type == MODEL_MONSTER01 + 12)
		{
			so->Position[2] += 150.0f; // Dịch chuyển lên cao 100 đơn vị
			CreateEffect(MODEL_SNOW1, so->Position, so->Angle, so->Light, 0, to);
			so->Position[2] -= 150.0f; // Trả về giá trị ban đầu
		}
		else if (so->Type == MODEL_MONSTER01 + 134)
		{
			so->Position[2] += 150.0f; // Dịch chuyển lên cao 100 đơn vị
			CreateEffect(MODEL_WOOSISTONE, so->Position, so->Angle, so->Light, 0, to);
			so->Position[2] -= 150.0f; // Trả về giá trị ban đầu
		}
		else if (so->Type == MODEL_MONSTER01 + 138)
		{
			vec3_t vLight;
			Vector(1.0f, 1.0f, 1.0f, vLight);

			so->Position[2] += 150.0f; // Dịch chuyển lên cao 100 đơn vị
			CreateEffect(MODEL_EFFECT_SAPITRES_ATTACK, so->Position, so->Angle, vLight, 0, to);
			so->Position[2] -= 150.0f; // Trả về giá trị ban đầu
		}
		else
		{
			so->Position[2] += 150.0f; // Dịch chuyển lên cao 100 đơn vị
			CreateEffect(BITMAP_ENERGY, so->Position, so->Angle, so->Light, 0, to);
			so->Position[2] -= 150.0f; // Trả về giá trị ban đầu
	...
	case MASTER_SKILL_ADD_POISON_IMPROVED:
		if (so->Type == MODEL_PLAYER)
			CreateEffect(MODEL_POISON, to->Position, so->Angle, so->Light);
		Vector(0.4f, 0.6f, 1.f, so->Light);

		for (int i = 0; i < 10; i++)
			CreateParticle(BITMAP_SMOKE, to->Position, so->Angle, so->Light, 1);
	...
	case MASTER_SKILL_ADD_FORCE_WAVE_IMPROVED:
		...
		CreateEffect(MODEL_WAVES, vec3_t{ so->Position[0], so->Position[1], so->Position[2] + 100.0f }, so->Angle, so->Light, 1);
		CreateEffect(MODEL_WAVES, vec3_t{ so->Position[0], so->Position[1], so->Position[2] + 100.0f }, so->Angle, so->Light, 1);
		CreateEffect(MODEL_PIERCING2, vec3_t{ so->Position[0], so->Position[1], so->Position[2] + 100.0f }, so->Angle, so->Light);
	...
	case MASTER_SKILL_ADD_FIRE_BURST_ENHANCED:
	{
		...
		BMD* b = &Models[so->Type];
		VectorCopy(so->Position, b->BodyOrigin);
		b->BodyScale = so->Scale;
		b->CurrentAction = so->CurrentAction;
		vec3_t p, Position;
		vec3_t Light;
		vec3_t Angle = { 0.f, 0.f, so->Angle[2] };
		vec3_t Pos = { 0.f, 0.f, (to->BoundingBoxMax[2] / 1.f) };
		Vector(80.f, 0.f, 20.f, p);
		b->TransformPosition(so->BoneTransform[0], p, Position, true);
		Angle[2] = so->Angle[2] + 90;
		...
		CreateEffect(MODEL_DARKLORD_SKILL, Position, so->Angle, Light, 0);
		CreateEffect(MODEL_DARKLORD_SKILL, Position, so->Angle, Light, 1);
	...
	case AT_SKILL_SPEAR:	// Ã¢Âî¸£±â
		if (sc->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(Hero->Helper.Type - 1171) == 5)
	...
	case AT_SKILL_SPACE_SPLIT:
		CreateEffect(MODEL_PIER_PART, so->Position, so->Angle,so->Light, 2, to);
		if ((sc->Helper.Type >= MODEL_HELPER + 2 && sc->Helper.Type <= MODEL_HELPER + 4) || gCustomPet2.GetInfoPetType(sc->Helper.Type - 1171) == 6 && !sc->SafeZone)
		{
			SetAction(so, PLAYER_ATTACK_RIDE_STRIKE);
		}
	...
	case MASTER_SKILL_ADD_GREATER_CRITICAL_DAMAGE_IMPROVED:
		...
		else if ((sc->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(Hero->Helper.Type - 1171) == 5) && !sc->SafeZone)
		{
			SetAction(so, PLAYER_FENRIR_ATTACK_MAGIC);
		}
		else
	...
	case MASTER_SKILL_ADD_SWORD_POWER_MASTERED:
	{
		...
		case MODEL_HELPER + 37:
			...
			break;
		default:
			if (gCustomPet2.GetInfoPetType(Hero->Helper.Type - 1171) == 5)
			{
				SetAction(so, PLAYER_SKILL_SLEEP_FENRIR);
			}
			else
	...
	if (so->Type == MODEL_PLAYER)
	{
		if (sc != Hero)
		{
			...
			case AT_SKILL_BLAST_CROSSBOW4:
				SetPlayerAttack(sc);
				break;   
			case AT_SKILL_BLAST_POISON: // Mưa Độc Tố DW
			case MASTER_SKILL_ADD_DECAY_IMPROVED:
			{
				vec3_t Position, Pos , Light;
				Position[0] = (float)(sc->SkillX + 2.2f) * TERRAIN_SCALE;
				Position[1] = (float)(sc->SkillY + 0.5f) * TERRAIN_SCALE;
				Position[2] = RequestTerrainHeight(Position[0], Position[1]);

				for (int i = 0; i < 5; ++i)  // Số lượng hạt độc rơi xuống
				{
					Pos[0] = Position[0];
					Pos[1] = Position[1];
					Pos[2] = Position[2] + 700.0f - (rand() % 150);  // 🛑 Rơi từ cao hơn (600)

					Vector(0.5f, 0.3f, 0.2f, Light);

					CreateEffect(MODEL_FIRE, Pos, so->Angle, Light, 6, NULL, i + 1);
				}
			...
			{
				vec3_t Position, Pos, Light;
				Position[0] = (float)(sc->SkillX + 0.5f) * TERRAIN_SCALE;
				Position[1] = (float)(sc->SkillY + 0.5f) * TERRAIN_SCALE;
				Position[2] = RequestTerrainHeight(Position[0], Position[1]);
				{
					int SkillIndex = FindHotKey((sc->Skill));
					CreateEffect(MODEL_SKILL_BLAST, Position, so->Angle, so->Light, 0, so, so->PKKey, SkillIndex);
					CreateEffect(MODEL_SKILL_BLAST, Position, so->Angle, so->Light, 0, so, so->PKKey, SkillIndex);
				}
			...
			break;
			case AT_SKILL_EVIL_SPIRIT_UP_M: // Xoay Rồng Dw
			...
			case AT_SKILL_EVIL:
				...
				for (int i = 0; i < 4; i++)
				{
					...
					int SkillIndex = FindHotKey((sc->Skill));
					CreateJoint(BITMAP_JOINT_SPIRIT, Position, so->Position, Angle, 0, so, 80.f, so->PKKey, SkillIndex, so->m_bySkillSerialNum);
					CreateJoint(BITMAP_JOINT_SPIRIT, Position, so->Position, Angle, 0, so, 20.f);
				}
				if (sc == Hero)
				{
					++CharacterMachine->PacketSerial;
			...
			break;
			case AT_SKILL_ICE_UP: // Mưa Sao Băng DW
			case AT_SKILL_ICE_UP + 1:
			case AT_SKILL_ICE_UP + 2:
			case AT_SKILL_BLAST_FREEZE:
			{
				vec3_t TargetPosition, Pos, Light;
				TargetPosition[0] = (float)(sc->SkillX + 1.5f) * TERRAIN_SCALE;
				TargetPosition[1] = (float)(sc->SkillY + 0.5f) * TERRAIN_SCALE;
				...
				for (int i = 0; i < 10; ++i)
				{
					...
					CreateEffect(MODEL_BLIZZARD, Pos, so->Angle, Light, 0, NULL, i + 1);
				}
				if (sc == Hero)
				{
					++CharacterMachine->PacketSerial;
			...
			case MASTER_SKILL_ADD_FLAME_IMPROVED2:
				...
				Position[1] = (float)(sc->SkillY + 0.5f) * TERRAIN_SCALE;
				Position[2] = RequestTerrainHeight(Position[0], Position[1]);
				CreateEffect(BITMAP_FLAME, Position, so->Angle, so->Light, 0, so, so->PKKey, FindHotKey((sc->Skill)));
				if (sc == Hero)
				{
					++CharacterMachine->PacketSerial;
				}
			...
			case MASTER_SKILL_ADD_PENETRATION_IMPROVED:
				...
				break;
			case AT_SKILL_STORM:
				CreateEffect(MODEL_STORM, so->Position, so->Angle, so->Light, 0, so, so->PKKey, FindHotKey(AT_SKILL_STORM));
				...
				break;
			case AT_SKILL_FLASH:
				CalcAddPositionFix(so, -20.f, -90.f, 100.f, Position);
				CreateEffect(BITMAP_BOSS_LASER, Position, so->Angle, so->Light, 0, so, so->PKKey, FindHotKey(AT_SKILL_FLASH));
			...
			case AT_SKILL_HELL_FIRE_UP: //HellFire
			case AT_SKILL_HELL:
			case MASTER_SKILL_ADD_HELL_FIRE_IMPROVED:
				CreateEffect(MODEL_CIRCLE, so->Position, so->Angle, so->Light, 0, so, so->PKKey, FindHotKey((sc->Skill)));
				CreateEffect(MODEL_CIRCLE_LIGHT, so->Position, so->Angle, so->Light);

				if (gMapManager.InHellas() == true)
				{
					AddWaterWave((sc->PositionX), (sc->PositionY), 2, -1500);
				}
			...
			case AT_SKILL_INFERNO: // Inferno
			case MASTER_SKILL_ADD_INFERNO_IMPROVED1:
			case MASTER_SKILL_ADD_INFERNO_IMPROVED2:
				CreateInferno(so->Position);
				CreateEffect(MODEL_SKILL_INFERNO, so->Position, so->Angle, so->Light, 0, so, so->PKKey, FindHotKey((sc->Skill)));
				if (sc == Hero)
				{
					++CharacterMachine->PacketSerial;
				}		
			...
			case MASTER_SKILL_ADD_TWISTING_SLASH_IMPROVED2:
				...
				so->WeaponLevel = (BYTE)sc->Weapon[0].Level;
				CreateEffect(MODEL_SKILL_WHEEL1, so->Position, so->Angle, so->Light, 0, so, so->PKKey, FindHotKey((sc->Skill)));	
				if (SceneFlag != LOG_IN_SCENE)
					PlayBuffer(SOUND_SKILL_SWORD4);
			...
			case MASTER_SKILL_ADD_FIRE_SLASH_ENHANCED:
				if (so->Type == MODEL_PLAYER)
				{
					...
					if (sc->CheckAttackTime(1) || sc->CheckAttackTime(2))
					{
						vec3_t Angle;
						Vector(1.f, 0.f, 0.f, Angle);
						CreateEffect(BITMAP_GATHERING, so->Position, so->Angle, so->Light, 1, so);
						sc->SetLastAttackEffectTime();
					}

					if (so->AnimationFrame >= 3.f && rand_fps_check(1))
					{
						so->PKKey = getTargetCharacterKey(sc, SelectedCharacter);

						PlayBuffer(SOUND_SKILL_SWORD3);

						if (iSkill == AT_SKILL_BLOOD_ATT_UP + 4)
						{
							vec3_t Offset1 = { so->Position[0] + 10.0f, so->Position[1], so->Position[2] };
							vec3_t Offset2 = { so->Position[0] - 10.0f, so->Position[1], so->Position[2] };
							CreateEffect(BITMAP_SWORD_FORCE, so->Position, so->Angle, so->Light, 1, so, so->PKKey, FindHotKey(iSkill));
							CreateEffect(BITMAP_SWORD_FORCE, so->Position, so->Angle, so->Light, 1, so, so->PKKey, FindHotKey(iSkill));
						...
						else
						{
							vec3_t Offset1 = { so->Position[0] + 10.0f, so->Position[1], so->Position[2] };
							vec3_t Offset2 = { so->Position[0] - 10.0f, so->Position[1], so->Position[2] };
							CreateEffect(BITMAP_SWORD_FORCE, so->Position, so->Angle, so->Light, 0, so, so->PKKey, FindHotKey(iSkill));
							CreateEffect(BITMAP_SWORD_FORCE, so->Position, so->Angle, so->Light, 0, so, so->PKKey, FindHotKey(iSkill));
			...
			case AT_SKILL_ALICE_LIGHTNINGORB:
			{
				...
				case MODEL_HELPER + 37:
					...
					break;
				default:
					if (gCustomPet2.GetInfoPetType(sc->Helper.Type - 1171) == 5)
					{
						SetAction(so, PLAYER_SKILL_LIGHTNING_ORB_FENRIR);
					}
					else
			...
			{
				CreateEffect(MODEL_EFFECT_FLAME_STRIKE,so->Position, so->Angle, so->Light, 0, so);
			...
			case AT_SKILL_GIGANTIC_STORM:
			{
				...
				for (int i = 0; i < 5; ++i)
				{
					...
					fAngle = so->Angle[2] + i * 72.f;
					Vector(0.f, 0.f, fAngle, vAngle);
					AngleMatrix(vAngle, Matrix);
					VectorRotate(vDirection, Matrix, vPosition);
					VectorAdd(vPosition, so->Position, vPosition);

					CreateEffect(BITMAP_JOINT_THUNDER, vPosition, so->Angle, so->Light);
				}
...
void ReceiveChainMagic(const BYTE* ReceiveBuffer)
{
	...
	case MODEL_HELPER + 37:
		...
		break;
	default:
		if (gCustomPet2.GetInfoPetType(pSourceChar->Helper.Type - 1171) == 5)
		{
			SetAction(pSourceObject, PLAYER_SKILL_CHAIN_LIGHTNING_FENRIR);
		}
		else
...
BOOL ReceiveDieExpLarge(const BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	if (addedExperience > 0)
	{
		...
		if (experienceType == eExperienceType_Master)
		{
			sprintf(Text, GlobalText[1750], addedExperience);
		}
		else
		{
			sprintf(Text, GlobalText[486], addedExperience);
		}
...
void ReceiveOption(BYTE* ReceiveBuffer)
{
	...
	const int	iTargetLevel = (Data->KeyTLevel >> 3) & 15;
	


	printf("============================ \n", Data->KeyR, byRLevel);
	printf("Data->KeyR %d Data->KeyRLevel %d \n", Data->KeyR, byRLevel);
	printf("Data->KeyT %d Data->KeyTLevel %d \n", Data->KeyT, iTargetLevel);
	printf("============================\n", Data->KeyR, byRLevel);
}
